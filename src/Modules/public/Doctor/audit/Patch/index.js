 ...
 app.patch(route, async (req, res) => {
  ...
  try {
   ...
   const { rows } = await client.query(
    'UPDATE public."Doctors" SET is_deleted = true WHERE 1=1 AND id = $1 RETURNING *',
    [id]
   );
   ...
   res.json({ Success: true, msg: 'Doctor was marked deleted successfully.', data: rows });
  } catch ({ message }) {
