 ...
 app.get(route, async (req, res) => {
  try {
   ...
   const { rows } = isPositiveInteger(id)
    ? await db.query(`SELECT * FROM public."V_Prescriptions" WHERE 1=1 AND ${clause} AND id=$1`, [id])
    : await db.query(
       `SELECT * FROM public."V_Prescriptions" WHERE 1=1 AND ${clause}   ${orderBy('id')} ${getLimitClause(limit)}`
      );
